...
class MetricsService
{
    ...
    /**
     * Average view duration (seconds) for voyage_visits events.
     */
    ...
    // -------------------------------------------------------------------------
    // 2. Voyage & destination metrics (direct table queries)
    // -------------------------------------------------------------------------
    ...
    /**
     * Number of logins in a date range.
     */
    public function getTotalLogins(string $startDate, string $endDate): int
    {
        $sql = 'SELECT COUNT(*) FROM user_logins WHERE login_time BETWEEN :start AND :end';
        return (int) $this->connection->fetchOne($sql, ['start' => $startDate, 'end' => $endDate]);
    }

    /**
     * Average logins per user in a date range.
     */
    public function getAverageLoginsPerUser(string $startDate, string $endDate): float
    {
        ...
        ';
        $avg = $this->connection->fetchOne($sql, ['start' => $startDate, 'end' => $endDate]);
        return (float) ($avg ?: 0);
    }

    /**
     * Number of distinct active users (login or event) in a date range.
     */
    ...
    /**
     * Average reservation value (confirmed only).
     */
    ...
    /**
     * Payment success rate (PAID vs total).
     */
    ...
    /**
     * Average resolution time (in hours) for resolved reclamations.
     */
    public function getAverageResolutionTimeHours(): float
    {
        ...
        ';
        $avg = $this->connection->fetchOne($sql);
        return (float) ($avg ?: 0);
    }
    ...
    public function getActiveOffersStats(): array
    {
        $sql = '
            SELECT 
                ...
                AVG(discount_percentage::numeric) as avg_discount
            FROM offers
